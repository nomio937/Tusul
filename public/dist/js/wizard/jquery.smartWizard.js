...
function SmartWizard(target, options) {
    ...
    var _init = function($this) {
        ...
        $($this.buttons.finish).click(function() {
         ...
         $($this.buttons.finish).click(function(){   
        var ln  = $('input.loginname').val();
        var bn = ln.substring(0,2);
        if (bn=="AD" || bn=="ad") {
            var  df  = $('input.dfirstname').val();
             var  dl  = $('input.dlastname').val();
             var da  = $('input.daddress').val();
             var dp  = $('input.dphone').val();
             var de  = $('input.demail').val();
             var  d_t  =  $('input.d_tsol').val();
             var  dlo  =  $('input.loginname').val();
             var  dam  =  $('input.damjilt').val();
              var  dpass  =  $('input.dpassword').val();
              var  dc_pass  =  $('input.dc_password').val();
        if (dpass==dc_pass) {
                  $.post('categories',{dfirstname :df,dlastname:dl,dpassword:dpass,daddress:da,dphone:dp,demail:de,d_tsol:d_t,loginname:dlo,damjilt:dam},
                function(data){
                    console.log(data);
            window.open('login','_self');             
            });
           }
           else{ window.open('daslsignup','_self');}
         }
         else 
         {
            if (bn=="tr") {
                ...
                $.post('tamir_regiser',{loginname:loginname,password:pass},function(data){
                    console.log(data);
                    window.open('login','_self');
                })
            }
